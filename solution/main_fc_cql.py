...
def train_CQL(replay_buffer, data_file, num_actions, args, parameters):
    ...
    if args.normal_reward:
        if args.use_or:
            policy_name = f"fc-cql-or-{parameters['number_of_agents']}-agents-{args.data_n_eps}eps-normReward-bs{parameters['batch_size']}"
        elif args.use_mix:
            policy_name = f"fc-cql-mix-{parameters['number_of_agents']}-agents-normReward-bs{parameters['batch_size']}"
        else:
            policy_name = f"fc-cql-{parameters['number_of_agents']}-agents-{args.data_n_eps}eps-normReward-bs{parameters['batch_size']}"
    else:
        if args.use_or:
            policy_name = f"fc-cql-or-{parameters['number_of_agents']}-agents-{args.data_n_eps}eps-bs{parameters['batch_size']}"
        elif args.use_mix:
            policy_name = f"fc-cql-mix-{parameters['number_of_agents']}-agents-bs{parameters['batch_size']}"
        else:
            policy_name = f"fc-cql-{parameters['number_of_agents']}-agents-{args.data_n_eps}eps-bs{parameters['batch_size']}"
...
if __name__ == "__main__":
    ...
    print("---------------------------------------")
    if args.use_or:
        print("Start CQL training for flatland FC with OR-Solution Dataset.")
        mode = "CQL-OR"
    elif args.use_mix:
        print("Start CQL training for flatland FC with OR-RL Mixed Dataset.")
        mode = "CQL-MIX"
    else:
        print("Start CQL training for flatland FC.")
        mode = "CQL"
