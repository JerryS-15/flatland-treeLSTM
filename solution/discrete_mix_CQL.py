...
class MultiAgentDiscreteCQL:
    def __init__(
        ...
        discount=0.99,
        tau=0.005,
        lr=3e-4,  # 1e-4 lower from 3e-4, improve stability
        target_update_freq=1000, # 200 lower from 1000
        # auto_alpha = False
    ):
        self.device = device
    ...
    def train(self, batch):
        ...
        bellman_loss_per_agent = F.mse_loss(chosen_q, target_q, reduction='none')
        bellman_loss = (bellman_loss_per_agent * mask_batch).sum() / mask_batch.sum()
        ...
        cql_penalty_per_agent = logsumexp_q - chosen_q
        cql_penalty = (cql_penalty_per_agent * mask_batch).sum() / mask_batch.sum()
