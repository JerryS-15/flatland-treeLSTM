...
def train_BC(replay_buffer, data_file, num_actions, args, parameters):
    device = torch.device("cuda:3" if torch.cuda.is_available() else "cpu:3")
    print(f"Using device: {device}")
    if args.normal_reward:
        if args.use_or:
            policy_name = f"bc-or-{parameters['number_of_agents']}-agents-{args.data_n_eps}eps-normReward-bs{parameters['batch_size']}"
        elif args.use_mix:
            policy_name = f"bc-mix-{parameters['number_of_agents']}-agents-normReward-bs{parameters['batch_size']}"
        else:
            policy_name = f"bc-{parameters['number_of_agents']}-agents-{args.data_n_eps}eps-normReward-bs{parameters['batch_size']}"
    else:
        if args.use_or:
            policy_name = f"bc-or-{parameters['number_of_agents']}-agents-{args.data_n_eps}eps-bs{parameters['batch_size']}"
        elif args.use_mix:
            policy_name = f"bc-mix-{parameters['number_of_agents']}-agents-bs{parameters['batch_size']}"
        else:
            policy_name = f"bc-{parameters['number_of_agents']}-agents-{args.data_n_eps}eps-bs{parameters['batch_size']}"
    policy_path = f"./policy/{policy_name}"

    policy = discrete_BC.MultiAgentBehaviorCloning(
        num_actions,
        device=device
    )
    ...
    while training_iters < args.max_timesteps:
        ...
        wandb.log({
            "BC/total_loss": avg_metrics["total_loss"],
            "BC/i_loss": avg_metrics["i_loss"],
            "BC/imt_max": avg_metrics["imt_max"]
        }, step=training_iters)

        print(f"[BC] Iteration: {training_iters} | Total Loss: {avg_metrics['total_loss']:.3f}")
...
if __name__ == "__main__":
    ...
    print("---------------------------------------")
    if args.use_or:
        print("Start BC for flatland TreeLSTM with OR-Solution Dataset.")
        mode = "BC-OR"
    elif args.use_mix:
            print("Start BC for flatland TreeLSTM with OR-RL Mixed Dataset.")
            mode = "BC-MIX"
    else:
        print("Start BC for flatland TreeLSTM.")
        mode = "BC"
    ...
    train_BC(replay_buffer, data_file, num_actions, args, parameters)
