...
from bcqActor import Actor  # Assuming this is where your BCQ Actor is defined
...
def get_model_path(n_agents):
    if n_agents <= 50:
        model_path = "policy/bcq-phase-III-50.pt"
    elif n_agents <= 80:
        model_path = "policy/bcq-80.pt"
    elif n_agents <= 100:
        model_path = "policy/bcq-100.pt"
    else:
        model_path = "policy/bcq-200.pt"
    return model_path


def get_args():
    parser = ArgumentParser(
        description="A multi-agent reinforcement learning solution to Flatland."
    )
    parser.add_argument(
        "--nr",
        "--no-render",
...
if __name__ == "__main__":
    ...
    print(f"Loaded BCQ model from {model_path}")
