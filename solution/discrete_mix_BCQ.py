...
class MultiAgentDiscreteBCQ:
    ...
    def train(self, batch):
        ...
        q_loss = (q_loss * mask_batch).sum() / mask_batch.sum()
        i_loss = F.nll_loss(imt_log.view(-1, self.num_actions), actions.view(-1), reduction='none')
        i_loss = i_loss.view(mask_batch.shape)
        i_loss = (i_loss * mask_batch).sum() / mask_batch.sum()
        logit_penalty = i_logits.pow(2).sum(dim=-1)  # [batch, 20]
        logit_penalty = (logit_penalty * mask_batch).sum() / mask_batch.sum()
        # total_loss = q_loss + i_loss + 1e-2 * i_logits.pow(2).mean()
        total_loss = q_loss + i_loss + 1e-2 * logit_penalty
