...
from nn.fc_bcq import BCQNetwork  # <- 使用之前改好的 FC 版本
import copy
...
class MultiAgentDiscreteBCQ:
    ...
    def _process_forest(self, forest):
        """
        将原来的 forest 转换成 agents_state。
        简单方式：取每个 agent 的根节点
        forest: [batch, n_agents, num_nodes, node_sz]
        返回: [batch, n_agents, node_sz]
        """
        return forest[:, :, 0, :]
    ...
    def train(self, batch):
        ...
        with torch.no_grad():
            ...
            q_target, _, _ = self.Q_target(next_agents_attr, next_agents_state)
            max_next_q = q_target.gather(-1, next_action).squeeze(-1)
            target_q = rewards + (1.0 - dones) * self.discount * max_next_q

        q_pred, imt_log, i_logits = self.Q(agents_attr, agents_state)
