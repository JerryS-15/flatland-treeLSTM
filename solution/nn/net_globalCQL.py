...
class CQLNetwork(nn.Module):
    def __init__(self):
        ...
        self.global_q_net = nn.Sequential(
            nn.Linear(fp.n_agents * (ns.hidden_sz + ns.tree_embedding_sz + fp.action_sz), 2 * ns.hidden_sz),
            nn.GELU(),
            nn.Linear(2 * ns.hidden_sz, ns.hidden_sz),
            nn.GELU(),
            nn.Linear(ns.hidden_sz, 1)  # <== Output scalar V(s)
        )
    
    def actor(self, agents_attr, forest, adjacency, node_order, edge_order):
        ...
        logits = self.actor_net(att_embedding)
        return logits

    def critic(self, agents_attr, forest, adjacency, node_order, edge_order, actions):
        batch_size, n_agents, num_nodes, _ = forest.shape
        device = next(self.parameters()).device
        adjacency = self.modify_adjacency(adjacency, device)

        tree_embedding = self.tree_lstm(forest, adjacency, node_order, edge_order)
        tree_embedding = tree_embedding.unflatten(0, (batch_size, n_agents, -1))[:, :, 0, :]
