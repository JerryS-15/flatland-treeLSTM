...
def train_CQL(replay_buffer, data_file, num_actions, args, parameters):
    ...
    if args.use_or:
        policy_name = f"cql-or-ALL-agents-normReward-bs{parameters['batch_size']}"
    elif args.use_mix:
        policy_name = f"cql-mix-ALL-agents-normReward-bs{parameters['batch_size']}"
    else:
        policy_name = f"cql-ALL-agents-normReward-bs{parameters['batch_size']}"
    policy_path = f"./policy/{policy_name}"

    policy = discrete_mix_CQL.MultiAgentDiscreteCQL(
        num_actions,
        device=device
    )
...
if __name__ == "__main__":
    ...
    if args.use_or:
        ...
        print("Start CQL training for flatland TreeLSTM with OR-RL Mixed Dataset.")
        mode = "CQL-MIX"
    else:
        print("Start CQL training for flatland TreeLSTM.")
        mode = "CQL"
    print("Training Details:")
    print("Current training is on mixed agents data.")
