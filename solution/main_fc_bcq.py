...
from fc_bcqActor import Actor
from replayBuffer import ReplayBuffer

import discrete_fc_bcq
...
def train_BCQ(replay_buffer, data_file, num_actions, args, parameters):
    ...
    if args.normal_reward:
        if args.use_or:
            policy_name = f"fc-bcq-or-{parameters['number_of_agents']}-agents-{args.data_n_eps}eps-normReward-bs{parameters['batch_size']}"
        elif args.use_mix:
            policy_name = f"fc-bcq-mix-{parameters['number_of_agents']}-agents-normReward-bs{parameters['batch_size']}"
        else:
            policy_name = f"fc-bcq-{parameters['number_of_agents']}-agents-{args.data_n_eps}eps-normReward-bs{parameters['batch_size']}"
    else:
        if args.use_or:
            policy_name = f"fc-bcq-or-{parameters['number_of_agents']}-agents-{args.data_n_eps}eps-bs{parameters['batch_size']}"
        elif args.use_mix:
            policy_name = f"fc-bcq-mix-{parameters['number_of_agents']}-agents-bs{parameters['batch_size']}"
        else:
            policy_name = f"fc-bcq-{parameters['number_of_agents']}-agents-{args.data_n_eps}eps-bs{parameters['batch_size']}"
    policy_path = f"./policy/{policy_name}"

    policy = discrete_fc_bcq.MultiAgentDiscreteBCQ(
        num_actions,
        device=device,
        lr=parameters["lr"]
    )
...
if __name__ == "__main__":
    ...
    print("---------------------------------------")
    if args.use_or:
        print("Start BCQ training for flatland FC with OR-Solution Dataset.")
        mode = "BCQ-OR"
    elif args.use_mix:
            print("Start BCQ training for flatland FC with OR-RL Mixed Dataset.")
            mode = "BCQ-MIX"
    else:
        print("Start BCQ training for flatland FC.")
        mode = "BCQ"
    ...
    wandb.init(
		...
		name=f"fc-{mode}_{parameters['number_of_agents']}agents_seed{args.seed}_{time.strftime('%m%d%H%M%S')}",
		config=parameters
	)
